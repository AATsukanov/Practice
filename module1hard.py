...
averaged = [sum(G) / len(G) for G in grades]

students = list(students)
students.sort()

my_dict = dict(zip(students, averaged))
print(my_dict)
'''
Добрый день!
А можно было пользоваться при решении for и zip?
Или надо было найти другое решение в рамках того, что мы прошли?
'''